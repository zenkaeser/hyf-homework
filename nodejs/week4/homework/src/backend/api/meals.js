...
router.post("/", (request, response) => {
  ...
  const post = [r.id, r.title, r.description, r.location, r.when, r.max_reservations, r.price, createdDate];

  pool.query("INSERT INTO meal VALUES (?,?,?,?,?,?,?,?)", post, (error, results) => {
    if (error) {
...
router.put("/:id", (request, response) => {
  ...
  pool.query("UPDATE meal SET title = ? WHERE id = ?", [r.title, id], (error, results) => {
    if (error) throw error;
