...
const expensiveListings = listings
  .filter(listing => listing.price>3000)
  .map(expensiveListings => expensiveListings);

const withParking = listings
  .filter(listing => listing.facilities.includes("Parkering"));
console.log(withParking);
...
function filterListings(listings, filter) {
  const listingForFarm = listings.filter(function(listing) {
    return listing.type.toLowerCase().includes(filter.type);
  })
  return listingForFarm;
}
